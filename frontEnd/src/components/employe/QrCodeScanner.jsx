...
export default function QRCodeScanner({ onClose, openModel }) {
  ...
  const parseQRTextToObject = (text) => {
    ...
    parts.forEach(part => {
      ...
      if (key === "Ticket_id") data.id = value;
    });
                ...
                >
                  {devices.map((device, index) => (
                    <option key={index} value={device.deviceId}>
                      Selected:&nbsp;{device.label || `Camera ${index + 1}`}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            {updateTicketG && (
              <div className="bg-gray-100 p-4 rounded-md mb-4 break-all">
                ...
                <p className="font-mono">Entry: {updateTicketG.entry_time}</p>
                <p className="font-mono">Exit: {updateTicketG.exit_time}</p>
