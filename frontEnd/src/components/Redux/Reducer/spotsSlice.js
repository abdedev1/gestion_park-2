...
const spotsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchSpots.pending, (state) => {state.status = 'loading';})
            .addCase(fetchSpots.fulfilled, (state, action) => {state.status = 'succeeded';state.spots = action.payload;})
            .addCase(fetchSpots.rejected, (state, action) => {state.status = 'failed';state.error = action.error.message;})
            .addCase(addSpot.fulfilled,(state,action)=>{state.spots.push(action.payload)})
            .addCase(updateSpot.fulfilled, (state, action) => {
              const index = state.spots.findIndex(employe => employe.id === action.payload.id);
              if (index !== -1) {
                state.spots[index] = action.payload;
              }
            })            
            .addCase(deleteSpot.fulfilled, (state, action) => {
                    state.spots = state.spots.filter(employe => employe.id !== action.payload);
                  })
            .addCase(getEmployeSpots.fulfilled, (state,action) =>{state.spots = action.payload})
    }
});
